...
if ($action = valider("action")) {
    switch ($action) {
            ...
            if ($idLesson = valider("lesson", "GET")) {
                if (SelectLessonGroupEditUser(valider("idUser", "SESSION"), $idLesson)) {
                    if (valider("connecte", "SESSION")) {
                        DeleteQPracticeById($idLesson);
                        DeleteTeamsLessonLinksByLesson($idLesson);
        ...
        case 'Valider':
            if ($idLesson = valider("lesson", "GET")){
                if (($title = valider("new-name", "GET")) && ($description = valider("new-description", "GET"))) {
                    if (SelectLessonGroupEditUser(valider("idUser", "SESSION"), $idLesson)) {
                        if (valider("connecte", "SESSION")) {
                            UpdateLessonTitle($idLesson, $title);
                            UpdateLessonDesc($idLesson, $description);
                        }
                    }
                }
            }
            if ($idLesson = valider("lessonId", "POST")) {
                printf("work");
                if ($text = valider("lesson", "POST")) {
                    if (valider("connecte", "SESSION")) {
                        UpdateLessonText($idLesson, $text);
                        printf("work");
                    }
                }
            }
